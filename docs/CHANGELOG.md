...
- Automatic update checks for packaged Windows builds with quiet downloads and an explicit restart prompt
- Settings update status so the current version, download progress, and install action are visible

### Changed

- Release uploads now include updater metadata and blockmaps for incremental update support
