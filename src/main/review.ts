...
function resolveScopePath(workspace: Workspace, repoRoot: string): string | null {
  ...
  const workingDirectory = normalizePath(workspace.workingDirectory || repoRoot)
  const normalizedRoot = normalizePath(repoRoot)
  if (workingDirectory === normalizedRoot) return null
  const relativePath = normalizePath(path.relative(normalizedRoot, workingDirectory))
